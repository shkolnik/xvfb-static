...
# triplication had already produced two divergences -- .# versus path:/src#, and
# cp -L versus install -m -- in scripts whose entire purpose is byte-determinism.
#
...
xvfb_static_build() {
  ...
  # Modes are set explicitly with install rather than inherited through cp,
  # so the archive does not depend on the store's modes or the caller's umask.
  docker run --rm \
