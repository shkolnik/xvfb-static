...
docker run --rm \
  ...
  "$image" sh -c "
    ...
    mkdir -p /src/out/glx-external-vulkan-alpha/$arch/package
    cp -RL /src/out/glx-external-vulkan-alpha/$arch/result/. /src/out/glx-external-vulkan-alpha/$arch/package/
