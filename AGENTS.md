...
closure-versus-licence audit (section 9), a two-host reproducibility
comparison, an SBOM, and digest pinning for the Alpine and Debian test
images. Section 11 tracks these.
...
| `.github/dependabot.yml` | Monthly GitHub Actions and Nix flake-input update checks. |
...
Two entry points still need `--impure`, legitimately: `nix-build-cached.sh`
resolving `cachix.nix`, and `test/manylinux-2-28-toolchain.sh` resolving
`test/glx-render.nix` and the toolchain files. These are `--file` invocations
that read `builtins.currentSystem` and `getFlake` a path, outside the flake's
own evaluation.
...
producing six artifacts. CI additionally runs a `bash -n` shell-syntax job over
every tracked script and a manylinux lock check, both of which gate the build
jobs.
...
branches. The build container digest lives in `build-image.txt` and is read by
every script and workflow, so there is nothing to keep synchronized by hand.
...
2. **Prove reproducibility.** Build twice from clean output directories, and
   ideally on two hosts, and compare archive SHA-256 values. A persistent Nix
   cache is fine; source output state must not leak between attempts. There is
   partial evidence: the July 2026 refactor left
   `xvfb-static-linux-x86_64.tar.gz` at
   `b386c9be62e5e3d5acd452d88454bbd634c73753a3fc47f669c9cf58f447cdf3` across
   several rebuilds with changed build-script and derivation text. That is
   same-host, same-cache evidence only.

3. **Promote or retire the external Vulkan alpha.** See the gate in section 10.
...
- **Pin GitHub Actions by commit SHA** (was gap 7). All nine references are
  pinned; Dependabot maintains them.
...
# Shell syntax, matching the CI job and release.sh
