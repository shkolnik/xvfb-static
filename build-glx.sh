...
case "$arch" in
  x86_64|aarch64) ;;
  *) echo "usage: ./build-glx.sh [x86_64|aarch64]" >&2; exit 2 ;;
esac
...
uid="$(id -u)"
gid="$(id -g)"
output="$root/out/glx-alpha/$arch"
mkdir -p "$output"
...
docker run --rm \
  ...
  "$image" sh -c "
    ...
    LC_ALL=C tar --sort=name --owner=0 --group=0 --numeric-owner \\
      --mtime=@315532800 \\
      -czf /src/out/glx-alpha/$arch/xvfb-static-glx-alpha-linux-$arch.tar.gz \\
      bin share
    cd /src/out/glx-alpha/$arch
    sha256sum xvfb-static-glx-alpha-linux-$arch.tar.gz > SHA256SUMS
    chown -R \"\$BUILD_UID:\$BUILD_GID\" /src/out/glx-alpha/$arch
  "
