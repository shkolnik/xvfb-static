...
toolchain.stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    # Scrub the probes with the same function that scrubs the shipped binary,
    # so a probe models the artifact rather than a laxer version of it. Every
    # probe needs it: the manylinux stdenv deliberately adds a build-only
    # -rpath into the sysroot so target-built helpers find their libc during
    # the build, and patchelf --remove-rpath deletes the DT_RUNPATH entry
    # while leaving its text in .dynstr.
